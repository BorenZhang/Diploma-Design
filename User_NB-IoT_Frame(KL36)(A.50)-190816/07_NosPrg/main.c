...
int main(void)
{
    ...
    BIOS_API_Init();
    gTimeSec=1588799218;         //默认2019-02-01 00:00:01
...
//    printf("AHL-IoT-GEC start...  \n");
    ...
    //新增温湿度传感器
    //adc_init(ADC_PIN9,16); //GEC15
    //DHT11_init(GPIOB_0); //GEC16
...
//     printf("NB-IoT communication OK!  \n");
    ...
    for(;;)
    {
        ...
        if (mTmp!=gTimeSec)             //1秒到
        {
        	...
        	printf("开关状态%d,%d,%d,%d\n",gpio_get(SWITCH_ELC),gUserData.isAutoOpenFan,human,gUserData.temp);
        	//自动开关风扇功能
